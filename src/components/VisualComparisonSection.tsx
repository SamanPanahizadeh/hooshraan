...
export const VisualComparisonSection: React.FC<VisualComparisonSectionProps> = () => {
  ...
  return (
    <section 
      ...
      className="relative space-y-8 sm:space-y-12 py-4 sm:py-8 overflow-hidden"
    ...
    >
      ...
      <div className="text-center max-w-3xl mx-auto space-y-3 sm:space-y-4 px-3">
        <h2 className="text-xl sm:text-4xl lg:text-5xl font-black text-white tracking-tight leading-snug">
          سازمان شما در کدام سمت قرار دارد؟
        </h2>
        <p className="text-xs sm:text-base lg:text-lg text-slate-300 max-w-xl mx-auto leading-relaxed font-normal">
      ...
      <div className="max-w-5xl mx-auto relative px-1 sm:px-14">
        ...
        <button
          ...
          className="absolute right-0 sm:right-2 top-1/2 -translate-y-1/2 z-20 w-8 h-8 sm:w-12 sm:h-12 rounded-full bg-slate-900/90 hover:bg-slate-800 text-white backdrop-blur-xl border border-white/20 shadow-xl flex items-center justify-center transition-all hover:scale-110 active:scale-95 cursor-pointer"
        >
        ...
        <button
          ...
          className="absolute left-0 sm:left-2 top-1/2 -translate-y-1/2 z-20 w-8 h-8 sm:w-12 sm:h-12 rounded-full bg-slate-900/90 hover:bg-slate-800 text-white backdrop-blur-xl border border-white/20 shadow-xl flex items-center justify-center transition-all hover:scale-110 active:scale-95 cursor-pointer"
        >
        ...
        <div className="relative">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="rounded-2xl sm:rounded-3xl bg-slate-800/70 border-2 border-white/80 p-3.5 sm:p-8 backdrop-blur-2xl shadow-2xl space-y-3 sm:space-y-6 min-h-[350px] sm:min-h-[400px] flex flex-col justify-between"
            >
              {/* هدر بالایی کارت */}
              <div className="flex items-center justify-between pb-2 sm:pb-3.5 border-b border-slate-700/60 shrink-0">
                <div className="flex items-center gap-2 sm:gap-3">
                  <span className="font-mono font-bold text-blue-300 bg-blue-500/20 px-2 py-0.5 sm:px-3 sm:py-1 rounded-md sm:rounded-xl border border-blue-400/30 text-[10px] sm:text-sm">
                    مقایسه {currentItem.number} از ۰۵
                  </span>
                  <span className="font-black text-white text-sm sm:text-lg xl:text-xl">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-2.5 sm:gap-4 items-stretch flex-1 my-auto">
                ...
                <div className="lg:col-span-5 p-3.5 sm:p-6 rounded-xl sm:rounded-2xl bg-slate-900/90 border-2 border-yellow-400 flex flex-col justify-between space-y-2 sm:space-y-4 shadow-sm min-h-[140px] sm:min-h-[250px]">
                  <div className="flex items-center justify-between shrink-0">
                    <span className="text-[10px] sm:text-xs font-bold px-2 py-0.5 sm:px-3 sm:py-1 rounded-md sm:rounded-lg bg-amber-500/15 text-amber-300 border border-amber-500/30">
                  ...
                  <div className="space-y-1 sm:space-y-2.5 flex-1 flex flex-col justify-center">
                    <h4 className="text-sm sm:text-lg xl:text-xl font-bold text-slate-100">
                  ...
                  {currentItem.without.flow ? (
                    <div className="pt-2 sm:pt-3.5 border-t border-slate-700/50 shrink-0">
                      <div className="flex flex-wrap items-center gap-1 sm:gap-1.5 text-[10px] sm:text-sm text-slate-300 font-mono" dir="ltr">
                        {currentItem.without.flow.map((step, sIdx) => (
                          <React.Fragment key={sIdx}>
                            <span className="px-1.5 py-0.5 sm:px-2.5 sm:py-1 rounded bg-slate-800 border border-slate-700 text-slate-200">
                              {step}
                            </span>
                ...
                <div className="lg:col-span-2 flex items-center justify-center py-0.5 sm:py-2 lg:py-0">
                  <div className="flex lg:flex-col items-center gap-1.5 sm:gap-2 text-center">
                    <div className="h-px lg:h-10 w-8 sm:w-12 lg:w-px bg-slate-700" />
                    <span className="w-6 h-6 sm:w-9 sm:h-9 rounded-full bg-slate-900 border border-slate-700 flex items-center justify-center text-[9px] sm:text-xs font-black text-slate-300 shadow-md shrink-0">
                      VS
                    </span>
                    <div className="h-px lg:h-10 w-8 sm:w-12 lg:w-px bg-slate-700" />
                  </div>
                ...
                <div className="lg:col-span-5 p-3.5 sm:p-6 rounded-xl sm:rounded-2xl bg-white border border-blue-100 flex flex-col justify-between space-y-2 sm:space-y-4 shadow-md min-h-[140px] sm:min-h-[250px]">
                  <div className="flex items-center justify-between shrink-0">
                  ...
                  {currentItem.withHoushran.flow && (
                    <div className="pt-2 sm:pt-3.5 border-t border-slate-100 shrink-0">
                      <div className="flex flex-wrap items-center gap-1 sm:gap-1.5 text-[10px] sm:text-sm font-mono" dir="ltr">
                        {currentItem.withHoushran.flow.map((step, sIdx) => (
                          <React.Fragment key={sIdx}>
                            <span className="px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-md bg-blue-600 text-white font-semibold">
                              {step}
                            </span>
                  ...
                  {currentItem.withHoushran.note && (
                    <div className="pt-1.5 sm:pt-2.5 border-t border-slate-100 flex items-start gap-1.5 sm:gap-2 text-[10px] sm:text-sm text-blue-900 font-semibold bg-blue-50/90 p-2 sm:p-3 rounded-lg sm:rounded-xl border border-blue-100 shrink-0">
        ...
        <div className="flex items-center justify-center gap-1.5 sm:gap-2 pt-2.5 sm:pt-4">
          {COMPARISON_PAIRS.map((_, dotIdx) => (
            <button
              key={dotIdx}
              onClick={() => setCurrentIndex(dotIdx)}
              className={`h-1.5 sm:h-2 rounded-full transition-all cursor-pointer ${
                currentIndex === dotIdx ? 'w-6 sm:w-8 bg-blue-500 shadow-sm' : 'w-1.5 sm:w-2 bg-white/20 hover:bg-white/40'
              }`}
      ...
      {/* بیانیه نهایی */}
      <div className="pt-20 sm:pt-40 pb-16 sm:pb-32 max-w-4xl mx-auto text-center space-y-4 sm:space-y-6 px-4 relative">
        ...
        <div className="space-y-3 sm:space-y-5">
          <h3 className="text-base sm:text-3xl lg:text-4xl font-black text-slate-400 leading-snug">
            مسئله فقط این نیست که AI را بلد باشید.
          </h3>
          <h3 className="text-xl sm:text-4xl lg:text-5xl font-black text-white leading-snug">
